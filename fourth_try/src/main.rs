...
fn read_file_contents(path: PathBuf) -> Result<String, Error> {
    ...
    let mut file: File = match File::open(path) {
        ...
        Err(io_error) => panic!("file doesn't exist!!! error: {}", io_error),
    };
    ...
    match file.read_to_string(&mut string) {
        Ok(_) => (),
        Err(io_error) => panic!("file reading error: {}", io_error),
    };
...
fn main() {
    ...
    if read_file_contents(PathBuf::from("src/main.rs")).is_ok() {
        println!("file content: {:?}", read_file_contents(PathBuf::from("src/main.rs")));
    }
